...
#define Version		  "efax v 0.9a-990411"
...
int puttrain ( TFILE *f, char *s, int n  )
{
  ...
  if ( ! err ) {
    ...
    /* allow 500ms (out of 1s total) for modem's training signal */
...
int main( int argc, char **argv)
{
  ...
  if ( ! header ) {
    char tmp [ MAXLINELEN ] ;
    now = time ( 0 ) ;
    strftime ( tmp, MAXLINELEN, "%%s   P. %%d", localtime ( &now ) ) ;
